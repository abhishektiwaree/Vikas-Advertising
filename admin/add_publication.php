...
if(isset($_POST['submit']) && $_POST['submit'] != ''){
	...
	if(isset($_POST['editsno']) && $_POST['editsno']!= ''){
		...
		$sql="UPDATE `publication` SET `heading`='".$_POST['heading']."',`date`='".$_POST['date']."' WHERE sno ='".$_POST['editsno']."'";
		...
		if (empty($errors)) {
			...
			$sql = "INSERT INTO publication(heading, date,file_path) VALUES ('".$_POST['heading']."','".$_POST['date']."','$newFileName')";
			...
			$uploadPath="publication/".$getsno.".".$fileExtension;
			if(mysqli_errno($db)){
				...
				if(move_uploaded_file($fileTmpName,$uploadPath)){
					execute_query("UPDATE publication SET file_path = '$uploadPath' where sno='$getsno'");
					$msg .='<p class="alert alert-success">File Upload  Successfully </p>';   
...
if (isset($_GET['edit'])) {
	$sql = 'select * from publication where sno=' . $_GET['edit'];
...
if (isset($_GET['del']) && $_GET['del']!='') {
	...
	if (file_exists($_GET['file_path'])) {
		...
		$sql = 'delete from publication where sno=' . $_GET['del'];
...
if(isset($_GET['status'])){
	...
	if($_GET['status']==1){
		...
		$toggle="UPDATE publication SET isactive ='0' where sno='".$_GET['sno']."'";
		...
		$toggle="UPDATE publication SET isactive ='1' where sno='".$_GET['sno']."'";
		if(execute_query($toggle)){
			// echo "updated toggle value";
		...
		<div class="card card-body">    
        	<div class="row d-flex my-auto ">  
				<form action="<?php echo $_SERVER['PHP_SELF'];?>" class="w-100" name="" enctype="multipart/form-data" method="POST" onSubmit="" >
					<?php echo $msg.'</br>' ?>
					<div class="bg-warning text-white p-2 my-2 " style="border-radius:5px;"><h3>Add Publication</h3></div>
